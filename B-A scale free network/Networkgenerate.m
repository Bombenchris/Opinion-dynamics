...
A = Scale_free(n, m0, m) + eye(n);    % need to add self loops???
...
% x_0 = betarnd(2,2,n,1); % uni- modal
% initial opinion obey symmetric uni-modal distribution given by a Beta distribution 

% Y = betarnd(2,10,n,1);  % bimodal
% x_0 = randsample([Y;1-Y],n); % initial opinion equal to Y with prob. 0.5 and equal to 1-Y with prob. 0.5

% Y = betarnd(2,10,n,1);  % 3 - modal
% Y_1 = randsample([Y;1-Y],n) .* 0.5; 
% x_0 = randsample([Y_1;1-Y_1],n);
...
T_w = 500; % about 600s for n = 1000 m = 2 
...
% for i_n = 1 : n
%     for i_t = 1 : n % swap to the i_t position, repeat T_w times.
%         for i = 1:T_w
%             x_0 = rand(n,1);  % uniform initial opinion from uniform distribution [0,1] at t = 0 ;
...
%             [x_s,I] = sort(x_0);
%             
%             x_0([I(i_t) i_n]) = x_0([i_n I(i_t)]); % swap node position
% 
...
subplot(4,1,3)
histfit(P_W,20,'normal')
subplot(4,1,4)
histfit(P_Final,20,'normal')
%subplot(3,1,3)
...
xlabel(ax3,'Centrality: indegree(SS Network)')
...
xlabel(ax4,'Centrality: pagerank(SS Network)')
ylabel(ax4,'Power')
...
xlabel(ax3,'Centrality: inclose_p(SSN)')
ylabel(ax3,'Power index')
...
xlabel(ax4,'Centrality: inbetw_p(SSN)')
